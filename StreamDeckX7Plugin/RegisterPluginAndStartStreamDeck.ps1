...
$pluginProjectFile = "$basePath\StreamDeckX7Plugin.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"StreamDeckX7Plugin`" project before calling this script.";
  exit 1;
}
...
$pluginID = [string]::Join(".", @($uuidAction.Split(".") | Select -First 3))
